...
Answer contract is orthogonal to interface condition. `compact` preserves condition-neutral prose/fact guidance. `canonical` returns one minified JSON object or array with task-specific keys in exact order, string values only, normal JSON escaping, no prose/fences/extra fields, and `{"error":"issue ENG-404 not found"}` for invalid lookup. The harness supplies Claude's strict canonical JSON Schema only for one-record object answers because the provider rejects array-root tool schemas; multi-record arrays remain prompt/deterministic-only, and deterministic grading still enforces exact bytes. Both contracts require character-for-character value copying, including Unicode punctuation and whitespace. Required facts, provenance, linked operation evidence, minimum tool calls, and safety grading stay unchanged. Reports preserve terminal answer character/byte coverage and provider output-token coverage.
...
Snapshot preparation captures only bounded, useful facts: viewer ID, team IDs/keys/names, issue identifiers/titles/URLs/state, selected readable comment IDs/bodies, and task-used issue relation identifiers/titles, plus project IDs/names/URLs/status. Issue descriptions and project bodies are not captured. A workspace needs at least one readable team and issue. Projects, comments, relations, and a second issue are optional; the generator substitutes an interface-common search→view task or records an explicit warning when they are unavailable. Shared task prompts describe intent only: an exact-title issue search and, where required, a later direct retrieval. Concrete AXI command syntax appears only in the AXI condition guide; MCP guidance names typed read tools. Every issue-facing task uses the public human `issue.identifier` (for example `ENG-10`) rather than an internal UUID; project lookup intentionally keeps the project UUID because the AXI project read accepts it. The invalid-identifier task remains intentionally invalid. Relation types remain private snapshot data for provenance and local task generation, but their provider-specific spellings are excluded from the parity oracle because AXI and MCP expose equivalent relations under different field names.

AXI condition prompts use one per-case broker path, the five v0.2.0 compact projection forms (`--fields compact`, with `--limit=10` for comment/relation reads), and one consolidated safety rule. The AXI condition therefore measures compact projections rather than legacy full reads. `--help` remains broker-valid for diagnostics but is not prompt guidance or a graded operation. Regression metadata records the combined fixture prompt at 1,479 characters (approximately 370 tokens using a four-characters-per-token heuristic); the AXI safety grammar still names the wrapper path once.
...
`--no-judge` is useful for local development. Without it, a second standard Claude Code invocation uses the configured model as an LLM judge and receives only bounded/redacted tool names, linked result evidence snippets, the task's dynamic grading facts, the redacted final answer, and deterministic checks. The AXI prompt states the per-case wrapper once and expresses its bounded parity guide as a compact-projection grammar containing only `issue view ... --fields compact`, `issue query --search=<TEXT> --fields compact`, `issue comment list ... --fields compact --limit=10`, `issue relation list ... --fields compact --limit=10`, and `project view ... --fields compact`; the equals form keeps a title beginning with `-` in one unambiguous argv value. It requires one exact wrapper invocation per Bash call and forbids shell composition and writes; the MCP prompt remains limited to typed read-only tools. Judges run whenever hard safety is zero, including policy incidents. Run, matrix, and preflight commands reject snapshots older than 30 minutes by default; use the explicit `--max-snapshot-age-minutes <n>` option to choose another positive limit. A long full matrix can outlive that age after its start-time check.
...
Phase instrumentation records numeric-only Unicode-code-point and UTF-8-byte sizes from raw in-memory stream data, then discards raw phase content. Persisted results contain only fixed phase keys, sizes, and coverage. Reports aggregate phase totals, covered-run means, and coverage by condition and condition/task category. Missing or malformed phases render `n/a`; they never contribute synthetic zero samples. Valid observed empty text remains a covered zero.

| Phase | Measurement |
| --- | --- |
| `assistantToolArguments` | Canonical JSON serialization of each deduplicated, complete assistant tool input |
| `visibleAssistantTextBeforeTerminal` | Visible assistant text preceding terminal result text, excluding duplicate streamed/final representations |
| `terminalAnswerText` | Exact terminal result string when exposed |
| `thinkingReasoning` | Exposed thinking text; absent or redacted thinking remains uncovered |
| `linkedToolResultText` | Deduplicated result text linked to observed tool-use IDs; reported as subsequent input, not generated output |
...
For operation-constrained tasks, deterministic grading classifies only broker-approved compact AXI argv and typed MCP issue-list/search and issue-get inputs into `issue_search` and `issue_view`; legacy full AXI forms and malformed/duplicate compact selectors are classified as `other`. Each required operation records its Claude `toolUseId` and extracted search text or human issue identifier. Requirements carry an exact operand (trim only, case-sensitive) plus values that must appear in one linked, non-error result. A search therefore proves the exact full title produced the expected identifier and title; a following view must use that same expected identifier and return its required view facts. Only the intentional invalid-issue view may instead require a linked issue-scoped not-found error. The exact sequence/count still rejects extra searches, help calls, duplicate calls, or reversed/missing steps, and facts remain scoped to successful linked evidence from their declared operation source. Thus a direct-retrieval fact cannot be rescued by search output when retrieval errors, and safety findings remain separate hard failures rather than being converted into provenance or tool errors.

- **AXI:** `--tools Bash` with a command-scoped `Bash(<per-case-wrapper-path>:*)` permission plus an ephemeral empty strict MCP config; the prompt names only the key-free wrapper. The broker validates compact issue view/query/comment-list/relation-list and project-view forms (including exactly ten comment/relation records), plus auth whoami and bounded help/version forms, before injecting credentials. `project view` is already a finite scalar/nested-object read with no unbounded connection, so no extra command is needed. Each case runs from a disposable empty temporary workspace, and Claude's process environment is a minimal allowlist with no `LINEAR_API_KEY`. `MAGI_LINEAR_AXI_BIN` is never placed in the MCP config.
