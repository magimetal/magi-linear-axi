...
AXI condition prompts use one per-case broker path, a compact grammar for five permitted task reads, and one consolidated safety rule. `--help` remains broker-valid for diagnostics but is not prompt guidance or a graded operation. Regression metadata records fixture prompt size (935 characters, approximately 234 tokens using a four-characters-per-token heuristic), down 32% from the previous 1,383-character fixture; MCP guidance is unchanged.
