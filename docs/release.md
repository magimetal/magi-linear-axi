...
1. Confirm branch, version `0.1.0` in `Cargo.toml`, matching `Cargo.lock`, and dated `CHANGELOG.md` section. Confirm `v0.1.0` does not exist locally, remotely, or on crates.io.
...
4. Inspect package contents and archive. Confirm expected source/docs files, ISC license and notices, no secrets, and no unintended files:
   ...
   tar -tf target/package/magi-linear-axi-0.1.0.crate
...
7. In isolated environment, install exact version and verify:
   ...
   cargo +1.87.0 install magi-linear-axi --version 0.1.0 --locked --root "$INSTALL_ROOT"
...
8. Create immutable annotated tag on published commit and GitHub release from reviewed changelog notes:
   ...
   ```sh
   git tag -a v0.1.0 -m "magi-linear-axi v0.1.0"
   git push origin v0.1.0
   gh release create v0.1.0 --verify-tag --title "magi-linear-axi v0.1.0" --notes-file <reviewed-release-notes.md>
   ```
